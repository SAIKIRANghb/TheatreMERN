...
const MoviePage = (props) => {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      try {
        const response = await fetch(`http://localhost:5001/movies/name/${movieParam}`);
        if (!response.ok) throw new Error('Failed to fetch movie');
        const movieData = await response.json();
        setMovie(movieData);

        const theatreRes = await fetch(`http://localhost:5001/theatres/${movieData.theatre}`);
  ...
  const fetchTimeSlots = async (movieId, screenId, theatreId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/slotsQuery?movieId=${movieId}&theatreId=${theatreId}`);
      ...
      await Promise.all(slotsData.map(async (slot) => {
        ...
        const screenResponse = await fetch(`http://localhost:5001/screens/${slot.screen}`);
