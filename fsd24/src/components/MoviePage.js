...
// //         console.log(movieData.screen);
// //         console.log(movieData.theatre);
...
//               <div className="time-slots">
//                 {selectedTimeSlots.length > 0 ? (
//                   selectedTimeSlots.map(({ time, screenNo }, index) => (
//                     <div key={`${time}-${screenNo}-${index}`} className="screen-times">
//                       <h3>Screen {screenNo || "N/A"}</h3>
//                       <Link className="time-button" to={`/${props.theatre}/${movieParam}/${screenNo}/${time}+${selectedDate}`}>
//                         {time}
//                       </Link>
...
const MoviePage = (props) => {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      try {
        const response = await fetch(`http://localhost:5000/movies/${movieId}`); // Updated to fetch movie by movieId
        if (!response.ok) throw new Error('Failed to fetch movie');
        const movieData = await response.json();
        setMovie(movieData);
              ...
              <div className="time-slots">
                {selectedTimeSlots.length > 0 ? (
                  selectedTimeSlots.map(({ time, screenNo }, index) => (
                    <div key={`${time}-${screenNo}-${index}`} className="screen-times">
                      <h3>Screen {screenNo || "N/A"}</h3>
                      <Link className="time-button" to={`/${props.theatre}/${movieId}/${screenNo}/${time}+${selectedDate}`}>
                        {time}
                      </Link>
                    </div>
                  ))
